...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # root_dir = os.getcwd()
        root_dir = '/Users/jmt/Data/Eye_Tracking/Groups/Laura'

    # Text splash
    print('')
    print('----------------------------------------')
    print('GeeTee Batch Eye Tracking Video Analysis')
    print('----------------------------------------')
    print('Version   : %s' % '0.1')
    print('Date      : %s' % datetime.datetime.now())
    print('Data root : %s' % root_dir)
    print('')
    
    # Load configuration information from root directory
    # If no config file exists, a default file is created
    print ('Loading configuration from <root_dir>/geetee.cfg')
    config = io.LoadConfig(root_dir)
    ...
    for subjsess in os.walk(videos_root).next()[1]:
        ...
        if os.path.isdir(subjsess_video_dir):
            ...
            io._mkdir(subjsess_results_dir)

            inext = config.get('VIDEO', 'inputextension')       
            cal_video  = os.path.join(subjsess_video_dir, 'cal' + inext)
            status = p.VideoPupilometry(cal_video, config)
            
            print('  Gaze Pupilometry')

            gaze_video = os.path.join(subjsess_video_dir, 'gaze' + inext)
            status = p.VideoPupilometry(gaze_video, config)
