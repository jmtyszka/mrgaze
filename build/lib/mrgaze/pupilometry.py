...
def PupilometryEngine(frame, cascade, cfg):
    ...
    if cfg.getboolean('PUPILDETECT', 'enabled'):
        ...
        x, y, w, h = json.loads(cfg.get('PUPILDETECT', 'manualroi'))
    ...
    if not blink:
        ...
        # TODO : Work out optimal way to remove glint area from ROI image
    ...
    if cfg.getboolean('OUTPUT', 'graphics'):
            ...
            if not blink:
                ...
                B = np.hstack( (pupil_bw * 255, glints_mask * 255) )
                quad = np.vstack( (A, B) )

                # Resample montage to 256 rows
                ny, nx = quad.shape
                new_ny, new_nx = 256, int(256.0 / ny * nx)
                quad_up = cv2.resize(quad, dsize=(new_nx, new_ny), interpolation=cv2.INTER_NEAREST)
                quad_up_rgb = cv2.cvtColor(quad_up, cv2.COLOR_GRAY2RGB)

            else:
                
                # Create blank 256 x 256 RGB image
                quad_up_rgb = np.zeros((256,256,3))
